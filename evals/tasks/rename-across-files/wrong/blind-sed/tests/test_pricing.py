from pricing.fx import fx_rate, inv_fx_rate
...
def test_direct_and_inverse_rates():
    assert fx_rate("EUR", "USD") == 1.0850
    assert round(fx_rate("USD", "EUR"), 6) == round(1 / 1.0850, 6)
    assert round(inv_fx_rate("EUR", "USD"), 6) == round(1 / 1.0850, 6)
